...
static inline void l2c_str_init(void* ptr) {
    uint8_t* p = (uint8_t*)ptr;
    p[0] = 0;      // 长度归零
    p[1] = '\0';   // C 语言字符串结尾
}

static inline void l2c_str_clear(void* ptr) {
    l2c_str_init(ptr);
}

// 压入单个字符 (极其适合串口单字节接收)
static inline void l2c_str_append_char(void* ptr, int max_len, char c) {
    uint8_t* p = (uint8_t*)ptr;
    if (p[0] < max_len - 1) {
        p[0]++;              // 长度 +1
        p[p[0]] = c;         // 写入字符
...
// 压入静态 C 字符串
static inline void l2c_str_append_cstr(void* ptr, int max_len, const char* str) {
    uint8_t* p = (uint8_t*)ptr;
...
static inline int l2c_str_len(void* ptr) {
    return ((uint8_t*)ptr)[0];
}

// 0-GC 极速字符串比较
static inline int l2c_str_eq(void* ptr, const char* cmp) {
    return strcmp((const char*)((uint8_t*)ptr + 1), cmp) == 0;
}

// 0-GC 极速整数解析 (用于解析 freq 433 命令)
static inline int l2c_str_to_int(void* ptr) {
